...
fn outou_build_regenerates_ui_kit_generated_output_byte_identically() {
    ...
    for original in &originals {
        ...
        assert_eq!(
            normalize_root(
                &normalize_generated_header(&original_text, "/ui-kit/"),
                &fixture_root
            ),
            normalize_root(
                &normalize_generated_header(&regenerated_text, "/ui-kit/"),
                &dir
            ),
...
///
/// This alone is not enough to compare a *committed* fixture against a
/// freshly regenerated temp copy: `root` is the current checkout's own
/// path (e.g. this repository's `tests/fixtures/workspace`, or CI's
/// `/home/runner/work/outou/outou/...`), but the header committed in the
/// fixture embeds whatever machine originally generated it (see
/// `normalize_generated_header`, which normalizes that away first).
fn normalize_root(text: &str, root: &Path) -> String {
    text.replace(&root.display().to_string(), "<root>")
}

/// Normalizes a generated file's `// Generated by Outou from file://…. Do
/// not edit.` header to a fixed, location-independent form, by replacing
/// everything from `file://` up to and including the first occurrence of
/// `anchor` (a path fragment identifying the fixture root within the
/// source tree, e.g. `"/ui-kit/"`) with `file://<root>` + `anchor`.
///
/// Unlike `normalize_root` (which only knows the *current* checkout's own
/// absolute path), this makes the header comparable even when the two
/// sides were generated on two different machines with two different
/// absolute prefixes — exactly the case for a fixture that was committed
/// on a developer's machine and is regenerated on CI: the committed
/// header carries the developer's absolute path (e.g.
/// `file:///Users/dev/outou/tests/fixtures/workspace/ui-kit/src/lib.rsx`),
/// while CI's regenerated header carries CI's own
/// (`file:///home/runner/work/outou/outou/tests/fixtures/workspace/ui-kit/src/lib.rsx`);
/// neither absolute prefix ever matches the other checkout's, so
/// `normalize_root` alone cannot make them equal. Text without a `file://`
/// header, or without `anchor` after it, is returned unchanged.
fn normalize_generated_header(text: &str, anchor: &str) -> String {
    let Some(scheme_start) = text.find("file://") else {
        return text.to_string();
    };
    let search_from = scheme_start + "file://".len();
    let Some(anchor_offset) = text[search_from..].find(anchor) else {
        return text.to_string();
    };
    let anchor_end = search_from + anchor_offset + anchor.len();
    format!(
        "{}file://<root>{}{}",
        &text[..scheme_start],
        anchor,
...
#[cfg(test)]
mod normalize_generated_header_tests {
    use super::normalize_generated_header;

    /// RED: a header carrying a foreign absolute path (as CI's checkout
    /// would produce) must normalize equal to the same header carrying
    /// the local developer's absolute path — this is exactly the
    /// comparison `outou_build_regenerates_ui_kit_generated_output_byte_identically`
    /// needs between the committed fixture (one machine's absolute path)
    /// and a freshly regenerated copy (another machine's absolute path).
    #[test]
    fn normalizes_foreign_and_local_absolute_paths_equal() {
        let local = "// Generated by Outou from file:///Users/kiyoshi/Documents/projects/outou-rs/outou/tests/fixtures/workspace/ui-kit/src/lib.rsx. Do not edit.\nuse outou::prelude::*;\n";
        let ci = "// Generated by Outou from file:///home/runner/work/outou/outou/tests/fixtures/workspace/ui-kit/src/lib.rsx. Do not edit.\nuse outou::prelude::*;\n";

        assert_eq!(
            normalize_generated_header(local, "/ui-kit/"),
            normalize_generated_header(ci, "/ui-kit/"),
        );
    }

    #[test]
    fn keeps_the_rest_of_the_file_byte_identical() {
        let text = "// Generated by Outou from file:///Users/kiyoshi/outou/tests/fixtures/workspace/ui-kit/src/widgets/button.rsx. Do not edit.\n#![allow(unused_braces)]\nuse outou::prelude::*;\n";

        let normalized = normalize_generated_header(text, "/ui-kit/");

        assert_eq!(
            normalized,
            "// Generated by Outou from file://<root>/ui-kit/src/widgets/button.rsx. Do not edit.\n#![allow(unused_braces)]\nuse outou::prelude::*;\n"
        );
    }

    #[test]
    fn leaves_text_without_a_file_header_unchanged() {
        let text = "use outou::prelude::*;\n";

        assert_eq!(normalize_generated_header(text, "/ui-kit/"), text);
    }
}
